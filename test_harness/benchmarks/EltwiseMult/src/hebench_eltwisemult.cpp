                                                           ...
                                                           const std::vector<hebench::APIBridge::WorkloadParam> &w_params) const
{
    ...
    if (bench_desc.workload == hebench::APIBridge::Workload::EltwiseMultiply)
    {
        try
        {
...
{
private:
    IL_DECLARE_CLASS_NAME(EltwiseMult::DataGeneratorHelper)

public:
    static void vectorEltwiseMult(hebench::APIBridge::DataType data_type,
...
private:
    ...
    static void vectorEltwiseMult(T *result, const T *a, const T *b, std::uint64_t elem_count)
    {
        ...
        std::transform(a, a + elem_count, // a[0..elem_count]
                       ...
                       std::multiplies<T>()); // eltwise mult
    }
};

void DataGeneratorHelper::vectorEltwiseMult(hebench::APIBridge::DataType data_type,
                                            void *result, const void *a, const void *b,
                                            std::uint64_t elem_count)
{
    switch (data_type)

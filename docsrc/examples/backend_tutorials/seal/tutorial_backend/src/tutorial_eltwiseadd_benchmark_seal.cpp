...
TutorialEltwiseAddBenchmark::Workload::SealBFVContext::SealBFVContext(int poly_modulus_degree)
{
...
TutorialEltwiseAddBenchmark::Workload::Workload(std::size_t vector_size)
{
    ...
    m_p_context   = std::make_shared<SealBFVContext>(TutorialEltwiseAddBenchmarkDescription::PolyModulusDegree);
}

std::vector<seal::Plaintext> TutorialEltwiseAddBenchmark::Workload::encodeVector(const std::vector<std::vector<std::int64_t>> &vec)
{
    std::vector<gsl::span<const std::int64_t>> vec_span(vec.begin(), vec.end());
    return encodeVector(vec_span);
}

std::vector<seal::Plaintext> TutorialEltwiseAddBenchmark::Workload::encodeVector(const std::vector<gsl::span<const std::int64_t>> &vec)
{
...
std::vector<seal::Ciphertext> TutorialEltwiseAddBenchmark::Workload::encryptVector(const std::vector<seal::Plaintext> &encoded_vec)
{
...
std::vector<seal::Ciphertext> TutorialEltwiseAddBenchmark::Workload::eltwiseadd(const std::vector<seal::Plaintext> &A,
...
std::vector<seal::Plaintext> TutorialEltwiseAddBenchmark::Workload::decryptResult(const std::vector<seal::Ciphertext> &encrypted_result)
{
...
std::vector<std::vector<int64_t>> TutorialEltwiseAddBenchmark::Workload::decodeResult(const std::vector<seal::Plaintext> &encoded_result)
{
    ...
    hebench::cpp::BaseBenchmark(engine, bench_desc, bench_params)
{
    ...
    // initialize original Workload (this initializes SEAL BFV context)
...
hebench::APIBridge::Handle TutorialEltwiseAddBenchmark::encode(const hebench::APIBridge::PackedData *p_parameters)
{
    ...
    for (std::size_t sample_i = 0; sample_i < clear_param.size(); ++sample_i)
    {
        ...
        assert(clear_param[sample_i].size() == w_params.n());
    }
    //! [benchmark encode preparation]

    //! [benchmark encode encoding]
    std::vector<seal::Plaintext> encoded = m_p_workload->encodeVector(clear_param);
    //! [benchmark encode encoding]
    //! [benchmark encode return]
    InternalParam<seal::Plaintext> retval;
                                         ...
                                         hebench::APIBridge::PackedData *p_native)
{
    ...
    const InternalParam<seal::Plaintext> &encoded_data =
        this->getEngine().retrieveFromHandle<InternalParam<seal::Plaintext>>(h_encoded_data, InternalParamInfo::tagPlaintext);
    //! [benchmark decode input_handle]
...
hebench::APIBridge::Handle TutorialEltwiseAddBenchmark::encrypt(hebench::APIBridge::Handle h_encoded_parameters)
{
    ...
    std::vector<seal::Ciphertext> encrypted = m_p_workload->encryptVector(encoded_parameter.samples);
    //! [benchmark encrypt encrypting]

    //! [benchmark encrypt return]
    InternalParam<seal::Ciphertext> retval;
...
hebench::APIBridge::Handle TutorialEltwiseAddBenchmark::decrypt(hebench::APIBridge::Handle h_encrypted_data)
{
    ...
    const InternalParam<seal::Ciphertext> &encrypted_data =
        this->getEngine().retrieveFromHandle<InternalParam<seal::Ciphertext>>(h_encrypted_data, InternalParamInfo::tagCiphertext);
    //! [benchmark decrypt input_handle]

    assert(encrypted_data.param_position == 0);

    //! [benchmark decrypt decrypting]
    std::vector<seal::Plaintext> encoded_data_samples = m_p_workload->decryptResult(encrypted_data.samples);
    //! [benchmark decrypt decrypting]
...
hebench::APIBridge::Handle TutorialEltwiseAddBenchmark::load(const hebench::APIBridge::Handle *p_local_data, uint64_t count)
{
    ...
    std::pair<std::vector<seal::Plaintext>, std::vector<seal::Ciphertext>> params;
    //! [benchmark load op_input_format]
                                                                ...
                                                                const hebench::APIBridge::ParameterIndexer *p_param_indexers)
{
    ...
    const std::pair<std::vector<seal::Plaintext>, std::vector<seal::Ciphertext>> &params =
        this->getEngine().retrieveFromHandle<std::pair<std::vector<seal::Plaintext>, std::vector<seal::Ciphertext>>>(
            h_remote_packed, InternalParamInfo::tagCiphertext | InternalParamInfo::tagPlaintext);

    // Looks familiar?
    const std::vector<seal::Plaintext> &A  = params.first;
    const std::vector<seal::Ciphertext> &B = params.second;
    //! [benchmark operate load_input]
    ...
    std::vector<seal::Ciphertext> result = m_p_workload->eltwiseadd(A, B);

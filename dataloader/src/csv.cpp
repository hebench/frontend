...
struct icsvstream:std::istringstream{
    struct csv_ws : std::ctype<char> {
        ...
        csv_ws(std::size_t refs = 0):ctype(make_table(),false,refs){}
    };
    static std::locale loc;
    template<class...Args> icsvstream(Args&&... args):
...
static void loadcsvdatafile(std::ifstream& ifs,std::vector<std::vector<T>>& v,size_t nlines,size_t skip=0){
    std::string line;
    while(skip--)std::getline(ifs,line);
...
ExternalDataset<T> ExternalDatasetLoader<T, E>::loadFromCSV
    ...
    while(std::getline(ifs,metaline)){
        if(metaline.at(0)=='#' or metaline.size()==0) continue;
        icsvstream iss(metaline);
        std::string tag,kind;
        size_t ix,nlines;
        iss>>tag>>ix>>nlines>>kind;
        std::vector<std::vector<std::vector<T>>> *u;
        if(tag=="input") u = &eds.inputs;
        if(tag=="output") u = &eds.outputs;
        if(ix>u->size()) u->resize(ix);
...
template<typename T, typename E> 
void ExternalDatasetLoader<T, E>::exportToCSV
    (const std::string& filename  
